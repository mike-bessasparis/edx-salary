...
```
## Executive Summary
The goal of this project is to reproduce results for this data set cited in a 1996 paper, Improving the Accuracy of Naive-Bayes Classifiers: a Decision-Tree Hybrid.[^1]  A summary of the results is found in the file adult.names.

The key steps are importing the data set, cleaning the data set, creating and tuning a model, and evaluating the model against an "unknown" data set.  I split the data set into a train_set and a test_set in order to avoid making any modelling decisions with the validation set.  Then model parameters are tuned and evaluated against the test_set.  Finally, the model makes predictions 

## Methods
The data required minimal cleaning.  I removed the observations with unknown values becuase the original folks did.

I choose models because....

First I build the data sets.  The data files consist of two files: adult.data and adult.validation.  I split the adult.data set into a
...
I obtained the data from https://archive.ics.uci.edu/ml/datasets/census+income.  This data was oringally extracted from the 1994 census bureau database.  Original Donor to the UCI is Ronny Kohavi and Barry Becker, Data Mining and Visualization Silicon Graphics, e-mail: ronnyk@sgi.com for questions.  Extraction was done by Barry Becker from the 1994 Census database. Data first cited in: 
