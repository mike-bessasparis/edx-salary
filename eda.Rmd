...
```
## Executive Summary
The goal of this project is to predict the salary given some U.S. Census data. I choos this project in order to attempt to match the prediction accuracy cited in a 1996 paper, Improving the Accuracy of Naive-Bayes Classifiers: a Decision-Tree Hybrid.[^1]  A summary of the results is found in the file adult.names.

The key steps are importing the data set, cleaning the data set, creating and tuning a model, and evaluating the model against a validation data set. In the original data set work the authors listed the top 3 performing models: NBTree, C4.5, and Naive-Bayes.  I choose a Random Forest, Neural Net, and xgbTree.  I split the data set into a train_set and a test_set in order to avoid making any modelling decisions with the validation set. Then model parameters are tuned and evaluated against the test_set.  Finally, the models make predictions for the validation set.  

## Methods
The data required minimal cleaning.  I removed the observations with unknown values becuase the original folks did.

First I build the data sets.  The data files consist of two files: adult.data and adult.validation.  I split the adult.data set into a train and test set.  I ignore the validation set until the final model run thru.

## Results
The original project used accuracy as evaluation function and so did I.  The original work achieved these top three results:

| Original Model | Accuracy | My Model | Accuracy |
|-------|----------|--------|----------|
| NBTree | 85.90+-0.28 | xgbTree | kj |
| C4.5  | 84.46+-0.30 | C5 | zz |
| Naive-Bayes | 83.88+-0.30 | Neural Network | hg |

## Conclusion
In conclusion, the models I created did about as well as the original.
...
Distribution of the Response Variable.  We see this is a somewhat unbalanced data set.
```{r}
...
It appears that Capital Gain can also help predicting Salary.  Capital Loss doesn't appear as helpful.
```{r}
...
dt <- table(mjb[, c("salary", "marital.status")])
chisq.test(dt)
...
We see that `workclass`, `occupation`, and `native.country` do not get a valid Chi-Square value because several factors in each do not contain enough samples.

Those that appear important to predicting salary are `education`, `marital.status`, `relationship`, and `sex`.  If we run into model performance constraints we can reduce the categorical predictors to these.
