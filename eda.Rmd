...
We apply a Chi-Square test for independence for each variable with `salary`.  We calculate the Cramer's V value for each variable against `salary`.  We see that `workclass`, `occupation`, and `native.country` do not get a valid Cramer's V value since they fail to produce a correct Chi-Square test.

Those variable that are important to predicting salary are `education`, `marital.status`, `relationship`, and `sex`.  If we run into model performance constraints we can reduce the categorical predictors to these.  
